...
func _on_threat_changed(new_threat_level: int) -> void:
	text = "Start Wave \nThreat: %d/%d" % [new_threat_level, Threat.threat_needed_for_wave_round(Game.wave_round)]	
	pass

func update_button_state() -> void:
	text = "Start Wave \nThreat: %d/%d" % [Threat.threat_level, Threat.threat_needed_for_wave_round(Game.wave_round)]